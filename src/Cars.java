...
class CarField extends JPanel { 
    ...
    final static Color barriercolor2 = new Color(80,150,80);    // A kind of green
    ...
    private int     barrieractive = 0;  // 0 ~ in active, 1 ~ active, 2 ~ shutting down;
    ...
    public void showBarrier(int active) {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (barrieractive > 0) {
            if (barrieractive == 1) g.setColor(barriercolor); else g.setColor(barriercolor2);
            if (barriertop) 
   ...
   public void clear(final Pos p){
    ...
    public Pos nextPos(int no, Pos pos) {
        ...
        if (no==0) {   // No. 0 is special, running its own tiny track
            ...
            if (pos.col==0  && pos.row < 10)       nxt.row++;
